...
class RadarCsvIntegrationTests(SimpleTestCase):
    ...
    def test_last_fights_keys_sorted_by_event_date(self):
        with TemporaryDirectory() as td:
            ...
            # Not used in this specific test but the module checks existence in the view
    ...
    def test_aggregate_for_fighter_metrics_inputs(self):
        with TemporaryDirectory() as td:
            ...
            # Two rows for same fight are ok (different rounds in real life),
            # but your aggregation currently sums everything it sees for that fighter in wanted fights.
            self._write(
                stats_csv,
                "EVENT,BOUT,FIGHTER,KD,SUB.ATT,SIG.STR.,TD,CTRL\n"
