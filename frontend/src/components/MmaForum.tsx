...
const NestedForumPage: React.FC = () => {
  ...
  // Update data when categories change
  useEffect(() => {
    setData(categories);
  }, [categories]);
        ...
        label={
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <IconButton
              size="small"
              color="error"
              onClick={() => deleteItem(API_REPLIES, reply.id, setReplies)}
            >
        ...
        label={
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <IconButton
              size="small"
              color="error"
              onClick={() => deleteItem(API_LIKES, like.id, setLikes)}
            >
              <Delete fontSize="small" />
            </IconButton>
          </Stack>
    ...
    (topic.posts ?? []).map((post) => (
      <TreeItem
        ...
        label={
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography>
              ✍️ {post.author_username}: {post.content}
            </Typography>
            ...
            <IconButton
              size="small"
              color="error"
              onClick={() => deleteItem(API_POSTS, post.id, setPosts)}
            >
              <Delete fontSize="small" />
            </IconButton>
        ...
        label={
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <IconButton
              size="small"
              color="error"
              onClick={() => deleteItem(API_TOPICS, topic.id, setTopics)}
            >
              <Delete fontSize="small" />
            </IconButton>
        ...
        label={
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <IconButton
              size="small"
              color="error"
              onClick={() => deleteItem(API_CATEGORIES, cat.id, setCategories)}
            >
              <Delete />
            </IconButton>
      ...
      <ReplyDialog
        ...
        onSave={async () => {
          ...
          try {
            ...
            loadData(API_REPLIES, setReplies);
          }
        }}
        saving={saving}
      />
